...
class Main extends React.Component {
  ...
  handleNextClick = () => {
    const curLevel = this.props.match.params.levelNum;
    if (curLevel < maxLevel) {
      this.setState({
        solved: false,
      });
      const nextLevel = parseInt(curLevel) + 1;
  ...
  check = () => {
    ...
    const stickerNodes = Array.from(this.boardEl.children);
    const zebraColors = []; //for plant colors test to work an element will need to be added to this array
    const leopardColors = [];
    const plantColors = [];

    while (stickerNodes.length > 0) {
      const curNode = stickerNodes.shift(); // start with the element at the front of the array
      ...
      const stickerColor = window.getComputedStyle(curNode).getPropertyValue('color');  //Set the color css property to stickercolor
  ...
  render() {
    const levelNum = this.props.match.params.levelNum;
